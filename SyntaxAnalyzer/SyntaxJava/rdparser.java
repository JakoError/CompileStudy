...
//        max_sign = Math.max(index, max_sign);
    ...
    public static boolean matchType(String type) {
        boolean match = index < rdparser.type.size() && rdparser.type.get(index).equals(type);
        //仅在匹配时增加（避免内部错失）
        //消费后才下一个
        if (match) index++;
        return match;
    ...
    public static boolean matchValue(String value) {
        boolean match = index < rdparser.value.size() && rdparser.value.get(index).equals(value);
        if (match) index++;
        return match;
    }
    ...
    public static void main(String[] args) {
        ...
        while (index < length) {
            ...
            if (compUnit == null) {
                ...
                System.out.println("函数追踪");
                for (StackTraceElement element : elements) {
                    System.out.println(element);
                }
                break;
            }
            compUnit.value = String.valueOf(++i);
            root.add(compUnit);
        }
        System.out.println("(默认各个compUnit分开，若不分开则取消compUnit中的注释)");
        for (Bean bean : root.beans)
            printTree(bean, 0);
    }

    public static void printTree(Bean bean, int layer) {
        System.out.println(bean);
        if (bean.beans != null)
            for (Bean b : bean.beans) {
                printSpace(layer);
                System.out.print("  |-");
                printTree(b, layer + 1);
            }
